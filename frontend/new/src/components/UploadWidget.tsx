...
export function UploadWidget() {
  ...
  const [isDragging, setIsDragging] = DragChecker();
  ...
  const handleSelectFile = useCallback(
    (event: Event) => {
      ...
      if (file) {
        setFileInfo({ ...fileInfo, name: file.name, size: file.size, type: file.type, blob: file });
      }
      setIsDragging(false);
    },
        ...
        <div className="upload-options">
          ...
          <div className="upload-option-group upload-file-name">
            ...
            <Input
              ...
              onChange={(event) => setFileInfo({ ...fileInfo, name: (event?.target as HTMLInputElement).value })}
            />
          </div>
