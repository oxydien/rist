...
import { baseRateLimitGuard } from "../_default";
import RequestQueue from "../requestQueue";

export function uploadStatus(uuid: string): Promise<UploadStatus | UploadError> {
...
export async function checkUploadStatus(uuid: string): Promise<string> {
  const queue = new RequestQueue(1, baseRateLimitGuard, 5);
  ...
  while (continueFlag) {
    try {
      ...
      if (typeof status === "object" && status !== null && "message" in status) {
        continueFlag = false;
        throw new Error(`Upload error while checking status: ${status.message}`);
      }
      ...
      const waitTime = Math.max(0, 4000 - (new Date().getTime() - time));
