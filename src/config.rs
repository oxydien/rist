...
use crate::routes::upload::error::{UploadError, UploadErrorKind};
      ...
      match key.as_str() {
        ...
        "upload" => {
          if let Some(upload_value) = value.as_object() {
            self.upload.max_size_bytes = upload_value["max_size_bytes"].as_i64().unwrap_or(0);
...
impl UploadConfig {
  pub fn check_size(&self, size: i64) -> Result<(), UploadError> {
