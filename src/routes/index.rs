...
use super::{RelaxedRateLimitGuard, TokenAuth};

#[get("/")]
pub async fn index<'r>(_brl: RocketGovernor<'r, RelaxedRateLimitGuard>) -> Option<NamedFile> {
  NamedFile::open("frontend/index.html").await.ok()
}

#[get("/assets/index.css")]
pub async fn index_style<'r>(_brl: RocketGovernor<'r, RelaxedRateLimitGuard>) -> Option<NamedFile> {
  NamedFile::open("frontend/index.css").await.ok()
}

#[get("/assets/global.css")]
pub async fn global_style<'r>(
...
pub async fn poppins_font<'r>(
  _brl: RocketGovernor<'r, RelaxedRateLimitGuard>,
) -> Option<NamedFile> {
  NamedFile::open("frontend/assets/Poppins-Variable.ttf")
    .await
    .ok()
}

#[get("/authorize")]
pub async fn authorization_page<'r>(
  _brl: RocketGovernor<'r, RelaxedRateLimitGuard>,
) -> Option<NamedFile> {
...
pub async fn authorization_style<'r>(
  _brl: RocketGovernor<'r, RelaxedRateLimitGuard>,
) -> Option<NamedFile> {
...
#[get("/dash/upload")]
pub async fn dash_upload_file<'r>(
  _brl: RocketGovernor<'r, RelaxedRateLimitGuard>,
  _auth: TokenAuth,
) -> Option<NamedFile> {
...
#[get("/assets/sha.js")]
pub async fn sha_js<'r>(_brl: RocketGovernor<'r, RelaxedRateLimitGuard>) -> Option<NamedFile> {
  NamedFile::open("frontend/assets/sha.js").await.ok()
}

#[get("/dash/youtube")]
pub async fn youtube_page<'r>(
  _brl: RocketGovernor<'r, RelaxedRateLimitGuard>,
  _auth: TokenAuth,
) -> Option<NamedFile> {
...
pub async fn chunk_upload_page<'r>(
  _brl: RocketGovernor<'r, RelaxedRateLimitGuard>,
  _auth: TokenAuth,
) -> Option<NamedFile> {
