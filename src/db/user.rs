...
impl UserDB {
  ...
  pub async fn sync_with_config(&self) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for user in state.config.accounts.user.iter() {
      self
        .insert(&user.name, user.kind.clone(), &user.token)
        .await?;
    }
...
impl User {
  ...
  pub fn has_permissions_to(&self, kind: PermissionKind) -> bool {
    match self.kind() {
      ...
      UserKind::YtOnly => {
        kind == PermissionKind::YoutubeDownload || kind == PermissionKind::MedalDownload
      }
...
#[derive(Copy, Clone, PartialEq, Serialize_repr)]
#[repr(u8)]
pub enum PermissionKind {
  FileUpload = 0b00000001,
  FileRemove = 0b00000010,
  YoutubeDownload = 0b00000100,
  MedalDownload = 0b00001000,
}
